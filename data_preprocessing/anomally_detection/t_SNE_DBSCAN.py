'''
수치형 데이터 X1~X5에 대해 표준화, t-SNE를 통해 차원 축소, DBSCAN으로 이상치 탐지 후 시각화
'''
import pandas as pd
...
# DBSCAN으로 탐지된 이상치 및 정상 데이터 분리
dbscan = DBSCAN(eps=0.5, min_samples=5)
dbscan_labels = dbscan.fit_predict(scaled_data)
tsne_df['is_outlier'] = dbscan_labels == -1
...
plt.savefig('tsne_dbscan_outlier.png')
