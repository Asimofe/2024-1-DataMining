...
# 모델 학습
grid_search_with_pca.fit(train_data_processed, y_train)

# 최적의 파라미터 출력
print("최적의 파라미터 (PCA 사용):", grid_search_with_pca.best_params_)

# 최적의 모델로 학습 데이터에 대한 예측
y_train_pred_with_pca = grid_search_with_pca.predict(train_data_processed)
...
y_pred_with_pca = grid_search_with_pca.predict(test_data_processed)

# 예측 결과를 CSV 파일로 저장 (PCA 사용)
predictions_with_pca = pd.DataFrame({'Predicted': y_pred_with_pca})
predictions_with_pca.to_csv('data/predictions_with_pca.csv', index=False)

print("예측 결과가 'predictions_with_pca.csv' 파일로 저장되었습니다.")
